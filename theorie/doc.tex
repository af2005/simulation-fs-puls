...
\subsection{Viele-Spalte}
Für eine beliebige Anzahl der Spalte brauchen wir eine Fallunterscheidung. Falls die Anzahl ungerade ist, ist genau in der Mitte ein Spalt und bei $\pm$nd weitere Spalte.
Für eine gerade Anzahl haben wir keinen Spalt in der Mitte, dafür aber bei $\pm$n(d/2).
Für N groß spielt diese Fallunterscheidung keine Rolle mehr und wird bei der weiteren Beschreibung nicht beachtet. Es sei immer ein Spalt in der Mitte.
...
\begin{align}
\Psi(\alpha_x) = \sum_{m=-n/2}^{n/2}\exp\Big(-ik\sin(\alpha_x)ma\Big) = \sum_{m=-n/2}^{n/2}\exp\Big(-ik\sin(\alpha_x)a\Big)^m
\end{align}

\subsection{Interferenzmuster eines periodischen 2D-Gitters}
